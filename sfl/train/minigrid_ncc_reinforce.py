...
from sfl.util.ncc_utils import scale_y_by_ti_ada, ScaleByTiAdaState, ti_ada, projection_simplex_truncated, tree_l2_norm
    ...
    def update_epoch(carry, _):
        def update_minibatch(train_state, minibatch):
            ...
            def loss_fn(params):
                ...
                actor_loss = (log_probs_pred * -A).mean()
                loss = actor_loss + critic_coeff * l_vf - entropy_coeff * entropy
                return loss, (l_vf, entropy)

            grad_fn = jax.value_and_grad(loss_fn, has_aux=True)
            loss, grads = grad_fn(train_state.params)
            if update_grad:
                train_state = train_state.apply_gradients(grads=grads)
            return train_state, (loss[0], *loss[1], tree_l2_norm(grads))
...
def main(config):
    ...
    def log_eval(stats, train_state_info):
        ...
        log_dict.update({
            "total_loss": stats["losses"][0][-1],
            "vf_loss": stats["losses"][1][-1],
            "entropy": stats["losses"][2][-1],
            "grad_norm": stats["losses"][3][-1],
            "meta_grad_norm": stats["meta_grad_norm"][-1],
            "meta_loss": stats["meta_loss"][-1]
        })
    ...
    def train_step(carry: Tuple[chex.PRNGKey, TrainState], _):
        ...
        metrics = {
            ...
            "meta_grad_norm": jnp.linalg.norm(grad)
        }
