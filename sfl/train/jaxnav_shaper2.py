...
class TrainState(BaseTrainState):
    shaping_vec: jnp.ndarray = struct.field(pytree_node=True)
    adv_state: BaseTrainState = struct.field(pytree_node=True)
    prev_scores: jnp.ndarray = struct.field(pytree_node=True)
...
def main(config):
    ...
    train_state = TrainState.create(
        ...
        tx=tx,
        sampler=sampler,
        prev_scores = jnp.zeros_like(sampler["scores"]),
        shaping_vec = jnp.zeros(config["SHAPER_ACTION_DIM"]),
    ...
    def train_step(carry, t):
        ...
        sample_dist = train_state.adv_state.apply_fn(train_state.adv_state.params, train_state.shaping_vec)
        ...
        init_obs, new_levels = jax.vmap(env.set_env_instance)(sampler["levels"])
        init_env_state = new_levels

        rng, _rng = jax.random.split(rng)
        disc_returns = sample_trajectories_rnn(
            _rng,
            env,
            train_state,
            ScannedRNN.initialize_carry(config["ued"]["PLR_PARAMS"]["capacity"] * env.num_agents, t_config["HIDDEN_SIZE"]),
            init_obs,
            init_env_state,
            t_config,
            config["ued"]["PLR_PARAMS"]["capacity"],
            t_config["GAMMA"]
        )[0][-1].mean(axis=0)

        @jax.grad
        def shaper_grad_fn(shaping_vec):

            sample_dist = train_state.adv_state.apply_fn(train_state.adv_state.params, shaping_vec)
            
            return disc_returns.T @ sample_dist

        train_state = train_state.replace(
            shaping_vec = train_state.shaping_vec + config["META_LR"] * shaper_grad_fn(train_state.shaping_vec)
        )
    ...
    def train_and_eval_step(runner_state, t):
        ...
        adv_grad_fn = jax.value_and_grad(lambda params: -scores.T @ train_state.adv_state.apply_fn(params, train_state.shaping_vec))
        ...
        test_metrics = {}
        test_metrics["shaper_loss"] = scores.T @ sample_dist
    ...
    runner_state = (rng, train_state.replace(prev_scores = init_scores))
