...
class ShaperRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        hidden, x = HandResetRNN()(hidden, x)
...
def main(config):
    ...
    rng, _rng = jax.random.split(rng)
    init_hstate = HandResetRNN.initialize_carry(1, t_config["HIDDEN_SIZE"])
    ...
    rng, _rng = jax.random.split(rng)
    adv_params = adv.init(_rng, jnp.zeros(config["SHAPER_ACTION_DIM"]))
    ...
    train_state = TrainState.create(
        ...
        shaper_hstate = HandResetRNN.initialize_carry(1, t_config["HIDDEN_SIZE"]),
    ...
    def train_and_eval_step(runner_state, t):
        ...
        sample_dist = train_state.adv_state.apply_fn(train_state.adv_state.params, shaping_vec)
        ...
        adv_grad_fn = jax.value_and_grad(lambda params: -new_sampler["scores"].T @ train_state.adv_state.apply_fn(params, shaping_vec))
