...
class TrainState(BaseTrainState):
    shaper_state: BaseTrainState = struct.field(pytree_node=True)
    shaper_hstate: jnp.ndarray = struct.field(pytree_node=True)
    adv_state: BaseTrainState = struct.field(pytree_node=True)
    adv_hstate: jnp.ndarray = struct.field(pytree_node=True)
    sampler: core.FrozenDict[str, chex.ArrayTree] = struct.field(pytree_node=True)


class HandResetRNN(nn.Module):
    @functools.partial(
        nn.scan,
    ...
    def __call__(self, carry, x):
        """Applies the module."""
        return nn.GRUCell(features=x.shape[-1])(carry, x)
...
class ActorCriticRNN(nn.Module):
    ...
    @nn.compact
    def __call__(self, hidden, x): # shaping_vec
        obs, dones = x
        embedding = nn.Dense(
...
class ShaperRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        x = nn.Dense(self.fc_dim_size)(x)
        x = nn.relu(x)

        hidden, x = nn.GRUCell(features=x.shape[-1])(hidden, x)

        x = nn.Dense(self.fc_dim_size)(x)
        x = nn.relu(x)

        x = nn.Dense(self.action_size)(x) # for SHAPER we would use a distrax multivariategaussian
...
class AdvRNN(nn.Module):
    ...
    def __call__(self, hidden, x):
        ...
        x = nn.Dense(self.buffer_size)(x)
        pi = jax.nn.softmax(x)

        return pi.squeeze(), hidden
...
def main(config):
    ...
    shaper = ShaperRNN(
        action_size=config["SHAPER_ACTION_DIM"],
        fc_dim_size=t_config["FC_DIM_SIZE"],
        hidden_size=t_config["HIDDEN_SIZE"],
    )
    adv = AdvRNN(
        fc_dim_size=t_config["FC_DIM_SIZE"],
        buffer_size=config["ued"]["PLR_PARAMS"]["capacity"],
    )
    ...
    rng, _rng = jax.random.split(rng)
    init_hstate = ShaperRNN.initialize_carry(1, t_config["HIDDEN_SIZE"])
    # NOTE: we will first try conditioning the shaper on the learnabilities from the previous iteration
    init_x = jnp.zeros((1, config["ued"]["PLR_PARAMS"]["capacity"]))
    shaper_params = shaper.init(_rng, init_hstate, init_x)

    rng, _rng = jax.random.split(rng)
    adv_params = adv.init(_rng, init_hstate, jnp.zeros(config["SHAPER_ACTION_DIM"]))

    if config["OPTIMISTIC"]:
        tx = optax.chain(
            optax.clip_by_global_norm(t_config["MAX_GRAD_NORM"]),
            optax.adam(linear_schedule),# ti_ada(vy0 = jnp.zeros(config["ued"]["PLR_PARAMS"]["capacity"]), eta=linear_schedule),
    ...
    train_state = TrainState.create(
        ...
        shaper_state = BaseTrainState.create(
            apply_fn = shaper.apply,
            params = shaper_params,
            tx = tx
        ),
        shaper_hstate = ShaperRNN.initialize_carry(1, t_config["HIDDEN_SIZE"]),
        adv_state = BaseTrainState.create(
            apply_fn = adv.apply,
            params = adv_params,
            tx = optax.adam(config["META_LR"])
        ),
        adv_hstate = ShaperRNN.initialize_carry(1, t_config["HIDDEN_SIZE"])
    )
    ...
    def train_and_eval_step(runner_state, t):
        ...
        rng, train_state, prev_scores = runner_state
        shaping_vec, shaper_hstate = train_state.shaper_state.apply_fn(train_state.shaper_state.params, train_state.shaper_hstate, prev_scores)
        sample_dist, adv_hstate = train_state.adv_state.apply_fn(train_state.adv_state.params, train_state.adv_hstate, shaping_vec)
        
        sampler = {**train_state.sampler, "scores": sample_dist}

        # TRAIN
        runner_state = (rng, train_state)
        (rng, train_state), metrics = jax.lax.scan(train_step, runner_state, jnp.arange(0, t_config["EVAL_FREQ"]) + t + 1, t_config["EVAL_FREQ"])
        
        # Update the level sampler
        levels = sampler["levels"]
        rng, _rng = jax.random.split(rng)
        scores, max_returns = learnability_fn(_rng, levels, config["ued"]["PLR_PARAMS"]["capacity"], train_state)
        # TODO: we will use the max_returns for the loss of the shaper

        rng, _rng = jax.random.split(rng)
        new_sampler = replace_fn(_rng, train_state, scores)

        adv_grad_fn = jax.value_and_grad(lambda params: -new_sampler["scores"].T @ train_state.adv_state.apply_fn(params, train_state.adv_hstate, shaping_vec)[0])
        
        @jax.value_and_grad
        def shaper_grad_fn(shaper_params):

            shaping_vec, _ = train_state.shaper_state.apply_fn(shaper_params, train_state.shaper_hstate, prev_scores)
            sample_dist, _ = train_state.adv_state.apply_fn(train_state.adv_state.params, train_state.adv_hstate, shaping_vec)

            return -max_returns.T @ sample_dist

        adv_loss, adv_grads = adv_grad_fn(train_state.adv_state.params)
        shaper_loss, shaper_grads = shaper_grad_fn(train_state.shaper_state.params)
        
        train_state = train_state.replace(
            shaper_state = train_state.shaper_state.apply_gradients(grads=shaper_grads),
            shaper_hstate = shaper_hstate,
            adv_state = train_state.adv_state.apply_gradients(grads=adv_grads),
            adv_hstate = adv_hstate,
            sampler = new_sampler
        )
        ...
        test_metrics = {}
        test_metrics["shaper_loss"] = shaper_loss
        test_metrics["adv_loss"] = adv_loss
        test_metrics["adv_entropy"] =  -jnp.dot(sample_dist, scores)
        ...
        return (rng, train_state, scores), test_metrics
    ...
    init_scores, _ = learnability_fn(_rng, sampler["levels"], config["ued"]["PLR_PARAMS"]["capacity"], train_state)
    runner_state = (rng, train_state, init_scores)
